...
"""
/***************************************************************************
 ToHNOR - versao 1.2 (verifica SGR)
                                 A QGIS plugin
...
class ToHNOR:
    ...
    def run(self):
        ...
        if result:
            ...
            #Verificar o SGR
            SGR_origem = self.camadaEntrada.crs()
            print(SGR_origem)
            SGR_destino = qgs.QgsCoordinateReferenceSystem("EPSG:4674")
            print(SGR_destino)
    
            diretorio_temporario = QTemporaryDir()
            caminho_temporario = os.path.join(diretorio_temporario.path(), 'camadaEntradaReproj.shp')
    
            if (SGR_origem != SGR_destino):   
                parametros = {
                    'INPUT': self.camadaEntrada,
                    'TARGET_CRS': 'EPSG:4674',
                    'OUTPUT': 'memory:camadaEntradaReprojTmp.shp'
                }
                reprojetado = processing.run('native:reprojectlayer', parametros)['OUTPUT']
              
                QgsProject.instance().addMapLayer(reprojetado)            
            
            #verificar se os atributos de entrada são do tipo float
                features = reprojetado.getFeatures() 
            else:
            ...
            if tipo_atributo_altitude == "nofloat":
               ...
               # exit()
                ...
                layer.commitChanges()
                if (SGR_origem != SGR_destino): 
                    QgsProject.instance().removeMapLayer(reprojetado)
